...
void main()
{
    ...
    if(GetLastSpellHarmful())
    {
        ...
        if (GetFactionEqual(oCaster, OBJECT_SELF) == TRUE)
        {
            ...
            DelayCommand(1.2, ActionDoCommand(ActivateDetermineCombatRound(OBJECT_INVALID)));
        ...
        if (MatchAreaOfEffectSpell(GetLastSpell()) == TRUE)
        {
            ...
            switch (nAI)
            {
                ...
                case X2_SPELL_AOEBEHAVIOR_FLEE:
                         ...
                         DelayCommand(1.2, ActionDoCommand(ActivateDetermineCombatRound(oCaster)));
                         bAttack = FALSE;
                         break;
        ...
        if( !GetIsFighting(OBJECT_SELF) && bAttack)
        {
            ...
            {
                ActivateDetermineCombatRound(oCaster);
            }
        }
    ...
    else
    {
        ...
        if ((GetIsObjectValid(oEnemy) == TRUE) && (GetIsInCombat() == TRUE))
        {
           // SpeakString("keep me in combat");
            ActivateDetermineCombatRound(oEnemy);
        }
    }
